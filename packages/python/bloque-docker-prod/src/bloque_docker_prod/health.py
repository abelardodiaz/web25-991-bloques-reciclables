"""Health checks for Docker Compose production services.

Extends the pattern from bloque-docker-dev with additional checks for
the app container and nginx reverse proxy. Uses Docker service names
as default hostnames.
"""
...
async def check_services(
    *,
    ...
    """Check health of Docker Compose production services.

    TCP check for PostgreSQL and Redis. HTTP health check for app, nginx,
    and optionally ChromaDB.

    Args:
        postgres_host: PostgreSQL host. Default 'postgres' (Docker service name).
        postgres_port: PostgreSQL port. Default 5432.
        redis_host: Redis host. Default 'redis' (Docker service name).
        redis_port: Redis port. Default 6379.
        app_url: App base URL. Default 'http://app:8000'.
        nginx_url: Nginx base URL. Default 'http://nginx:80'.
    ...
    Returns:
        ...
        Example: {"postgres": True, "redis": True, "app": True, "nginx": True}
    """
    tasks = [
        _check_tcp(postgres_host, postgres_port, timeout),
        _check_tcp(redis_host, redis_port, timeout),
        _check_http(f"{app_url.rstrip('/')}/health", timeout),
        _check_http(f"{nginx_url.rstrip('/')}/health", timeout),
    ]
    keys = ["postgres", "redis", "app", "nginx"]
