...
import httpx
import pytest
from bloque_docker_prod.health import (
    _check_http,
    _check_tcp,
    check_services,
    wait_for_services,
...
class TestCheckTcp:
    async def test_successful_connection(self):
        ...
        with patch("bloque_docker_prod.health.asyncio.open_connection") as mock_conn:
    ...
    async def test_connection_timeout(self):
        with patch("bloque_docker_prod.health.asyncio.wait_for") as mock_wait:
...
class TestCheckHttp:
    async def test_healthy_endpoint(self):
        ...
        with patch("bloque_docker_prod.health.httpx.AsyncClient", return_value=mock_client):
...
class TestCheckServices:
    async def test_all_healthy(self):
        with (
            patch("bloque_docker_prod.health._check_tcp", return_value=True),
            patch("bloque_docker_prod.health._check_http", return_value=True),
        ):
    ...
    async def test_postgres_down(self):
        ...
        with (
            patch("bloque_docker_prod.health._check_tcp", side_effect=mock_tcp),
            patch("bloque_docker_prod.health._check_http", return_value=True),
        ):
            result = await check_services()
...
class TestWaitForServices:
    ...
    async def test_becomes_healthy(self):
        ...
        with (
            patch("bloque_docker_prod.health._check_tcp", side_effect=flaky_check),
            patch("bloque_docker_prod.health._check_http", side_effect=flaky_check),
        ):
    ...
    async def test_timeout_raises(self):
        with (
            patch("bloque_docker_prod.health._check_tcp", return_value=False),
            patch("bloque_docker_prod.health._check_http", return_value=False),
        ):
    ...
    async def test_selective_services(self):
        ...
        with (
            patch("bloque_docker_prod.health._check_tcp", side_effect=only_redis),
            patch("bloque_docker_prod.health._check_http", return_value=False),
        ):
            result = await wait_for_services(
                services=["redis"],
