...
```bash
uv add bloque-multitenant
# or
pip install bloque-multitenant
```
...
from bloque_multitenant.rls import apply_rls
...
from bloque_multitenant.rls import TenantMiddleware
...
```python
from bloque_multitenant.context import get_current_tenant, set_current_tenant, clear_current_tenant
...
The typical setup combines `bloque-auth` JWT middleware with `bloque-multitenant`:

```python
from fastapi import FastAPI
from bloque_core.middleware import RequestIDMiddleware
from bloque_multitenant.rls import TenantMiddleware, apply_rls
