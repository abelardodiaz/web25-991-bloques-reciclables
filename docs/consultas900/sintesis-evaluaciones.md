...
| 6 | **Versionado automatizado** | Falta automatizacion de versiones en monorepo | Changesets adaptado para `uv publish` - sin esto "el monorepo colapsara" | (no profundiza en tooling) | Script propio en vez de Changesets/Release Please (overkill para Python+uv). scripts/version.py detecta cambios por paquete desde ultimo tag, determina bump (patch/minor/major) desde conventional commits, actualiza pyproject.toml, genera CHANGELOG.md. scripts/release.sh orquesta bump+commit+tag+publish | ALTA |
...
| 9 | **Competencia y diferenciacion** | No comunica ventaja clara vs alternativas | vs SaaS Pegasus/ShipFast: dependencias vs codigo fuente = mas rigido | vs Django/Supabase, tabla comparativa. Falta "killer feature" | Creado docs/COMPARATIVA.md con tabla honesta vs 6 alternativas. Renaming completo bloque-* -> ulfblk-* (nombre unico, libre en PyPI/npm, corto). 16 paquetes renombrados. Prerequisito para publicar en PyPI (#7) | MEDIA |
